...
def update_progress(step, message, progress_percent):
    """Update progress in session and print to terminal"""
    session['progress'] = {
        'step': step,
        'message': message,
    ...
    print(f"[{step.upper()}] {progress_percent}% - {message}")
...
def process_uploaded_csv(df):
    try:
        ...
        print(f"📊 Processing {len(df)} strategies from uploaded file...")
        with get_conn() as conn:
            with conn.cursor() as cur:
                ...
                conn.commit()
                print(f"✅ Successfully inserted {len(df)} strategies into database")
        
        update_progress('running_backtest', 'Running backtesting algorithms...', 70)
        print("🚀 Starting backtesting execution...")
        
        # Clear previous results before running new backtest
        print("🗑️ Clearing previous backtest results...")
        with get_conn() as conn:
            with conn.cursor() as cur:
        ...
        print("📈 Processing and aggregating backtest results...")
        
        update_progress('complete', 'Backtesting completed successfully!', 100)
        print("✅ Backtesting completed successfully!")
        
        duration = time.time() - start_time
        session['processing_duration'] = duration
...
def upload_file():
    ...
    if file and allowed_file(file.filename):
        ...
        try:
            ...
            print(f"📊 CSV file processed successfully: {len(df)} rows, {len(df.columns)} columns")
...
def results():
    ...
    duration = session.get('processing_duration', None)
    
    return render_template('results.html', results=results_df, analysis=analysis_df.to_dict('records'), top_strategies=top_strategies, duration=duration)
...
if __name__ == '__main__':
    print("🌟 Starting New_BackTest_Pulse Flask Application...")
